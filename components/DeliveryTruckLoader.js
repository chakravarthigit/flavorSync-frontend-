...
class DeliveryTruckLoader extends React.Component {
  constructor(props) {
    super(props);
    
    // Animation values
    this.truckMotion = new Animated.Value(0);
    this.roadAnimation = new Animated.Value(0);
  }

  componentDidMount() {
    this.startTruckAnimation();
    this.startRoadAnimation();
  }

  startTruckAnimation() {
    Animated.loop(
      Animated.sequence([
        Animated.timing(this.truckMotion, {
          toValue: 1,
          duration: 500,
  ...
  startRoadAnimation() {
    Animated.loop(
      Animated.timing(this.roadAnimation, {
        toValue: 1,
        duration: 1400,
        easing: Easing.linear,
        useNativeDriver: true,
  ...
  render() {
    // Interpolate the animation values
    const truckTranslateY = this.truckMotion.interpolate({
      inputRange: [0, 0.5, 1],
      outputRange: [0, 3, 0],
    });

    const roadTranslateX = this.roadAnimation.interpolate({
      inputRange: [0, 1],
      outputRange: [0, -350],
    });
      ...
      <View style={styles.container}>
        <View style={styles.loaderContainer}>
          <View style={styles.truckWrapper}>
            ...
            <Animated.View style={[
              styles.roadMarkersContainer,
              { transform: [{ translateX: roadTranslateX }] }
            ]}>
              <View style={styles.roadMarker1} />
              <View style={styles.roadMarker2} />
            </Animated.View>
...
const styles = StyleSheet.create({
  ...
  roadMarkersContainer: {
    position: 'absolute',
    bottom: 0,
    right: -50,
    flexDirection: 'row',
    width: 400, // Ensure it's wide enough for animation
  },
  roadMarker1: {
    position: 'absolute',
    width: 20,
  ...
  lampPostContainer: {
    position: 'absolute',
    bottom: 0,
    right: -180, // Start from off-screen
  },
  lampPost: {
    height: 90,
    width: 45,
